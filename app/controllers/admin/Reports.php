...
class Reports extends MY_Controller
{
    ...
    public function getBrandsReport($pdf = null, $xls = null)
    {
        ...
        $pp = "( SELECT pp.brand as brand, SUM( pi.quantity ) purchasedQty, SUM( pi.subtotal ) totalPurchase from {$this->db->dbprefix('products')} 
        pp left JOIN " . $this->db->dbprefix('purchase_items') . ' pi ON pp.id = pi.product_id
        ...
        if ($pdf || $xls) {
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select($this->db->dbprefix('brands') . '.id as id, ' . 
    ...
    // , SUM( pi.quantity ) purchasedQty, SUM( pi.subtotal ) totalPurchase 
    // , SUM( si.quantity ) soldQty, SUM( si.subtotal ) totalSale 
    ...
    public function getProductsReport($pdf = null, $xls = null)
    {
        ...
        $pp = "( SELECT product_id, p.date as date, p.created_by as created_by, SUM(CASE WHEN pi.purchase_id IS NOT NULL THEN quantity ELSE 0 END) as purchasedQty, SUM(quantity_balance) as balacneQty, SUM( unit_cost * quantity_balance ) balacneValue, SUM( (CASE WHEN pi.purchase_id IS NOT NULL THEN (pi.subtotal) ELSE 0 END) ) totalPurchase from 
        {$this->db->dbprefix('purchase_items')} pi LEFT JOIN 
        {$this->db->dbprefix('purchases')} p on p.id = pi.purchase_id WHERE pi.status = 'received' ";
    ...
    {
        $this->sma->checkPermissions('event');
        $this->data['error']      = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['events']     = $this->site->getAllEvents();
        // $this->data['warehouses'] = $this->site->getAllWarehouses();
        if ($this->input->post('start_date')) {
            $dt = 'From ' . $this->input->post('start_date') . ' to ' . $this->input->post('end_date');
        } else {
        ...
        $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => lang('register_report')]];
        $meta = ['page_title' => lang('register_report'), 'bc' => $bc];
        $this->page_construct('reports/register_events', $meta, $this->data);
    }
    //  public function getRegistersReport($pdf = null, $xls = null)
    // {
    //     $this->sma->checkPermissions('event', true);
    //     // $warehouse  = $this->input->get('warehouse') ? $this->input->get('warehouse') : null;
    //     $brand      = $this->input->get('brand') ? $this->input->get('brand') : null;
    //     $start_date = $this->input->get('start_date') ? $this->input->get('start_date') : null;
    //     $end_date   = $this->input->get('end_date') ? $this->input->get('end_date') : null;
    ...
    //     $pp = "( SELECT pp.category_id as category from {$this->db->dbprefix('products')} pp
    //             left JOIN " . $this->db->dbprefix('purchase_items') . ' pi ON pp.id = pi.product_id
    //             left join ' . $this->db->dbprefix('purchases') . ' p ON p.id = pi.purchase_id ';
    //     $sp = "( SELECT sp.category_id as category from {$this->db->dbprefix('products')} sp
    //             left JOIN " . $this->db->dbprefix('sale_items') . ' si ON sp.id = si.product_id
    //             left join ' . $this->db->dbprefix('sales') . ' s ON s.id = si.sale_id ';
    //     if ($start_date || $warehouse) {
    //         $pp .= ' WHERE ';
        ...
        //  else {
            ...
            $this->datatables
                ->select($this->db->dbprefix('categories') . '.id as cid, ' 
                    ...
                    SUM( COALESCE( sa.totalPurchase, 0 ) ) as TotalPurchase,
                    SUM( COALESCE( PSales.totalSale, 0 ) ) as TotalSales,
                    (SUM( COALESCE( PSales.totalSale, 0 ) )- SUM( COALESCE( sa.totalPurchase, 0 ) ) ) as Profit', false)
                ->from('categories')
                ->join($sp, 'categories.id = PSales.category', 'left')
                ->join($pp, 'categories.id = sa.category', 'left')
                ->group_start()->where('parent_id is NULL', null, false)->or_where('parent_id', 0)->group_end();

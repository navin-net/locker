...
class Products extends MY_Controller
{
    ...
    public function view($id = null)
    {
        ...
        $pr_details = $this->products_model->getProductByID($id);
        $this->sma->print_arrays($pr_details);
        // if (!$id || !$pr_details) {
        //     $this->session->set_flashdata('error', lang('prduct_not_found'));
        //     redirect($_SERVER['HTTP_REFERER']);
        // }
    ...
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('products'), 'page' => lang('products')], ['link' => '#', 'page' => lang('brands')]];
     ...
     public function import_brands()
    {
        ...
        if ($this->form_validation->run() == true && !empty($data) && $this->products_model->addBrands($data)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'products_model/import_brands', $this->data);
        }
    }

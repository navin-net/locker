...
class Shop_model extends CI_Model
{
    ...
    public function getEventSpeakerByID($id)
    {
        $query = $this->db->select("{$this->db->dbprefix('md_speakers')}.*")
        ->from('md_speakers')
        ->join('md_event_speakers','md_speakers.id=md_event_speakers.speaker_id')
        ->where('md_event_speakers.event_id',$id)
        ->get()
        ->result();
        return $query;
    }
    ...
    public function getNewsTags($new_id)
    {
        $tag = $this->db->select("{$this->db->dbprefix('md_tags')}.*")
        ->from('md_tags')
        ->join('md_news_tags','md_tags.id=md_news_tags.tag_id')
        ->where('md_news_tags.news_id',$new_id)
        ->get()
        ->result();
        return $tag;
    }

    public function getLastNewsDetail($limit)
    {
        $lastNews = $this->db->select("{$this->db->dbprefix('md_news')}.*")
        ->from('md_news')
        ->order_by('id','desc')
        ->limit($limit)
        ->get()
    ...
    public function getAchiveDate()
    {
        $date = $this->db->select("{$this->db->dbprefix('md_news')}.created_at")
        ->from('md_news')
    ...
    {
        $this->db->select("{$this->db->dbprefix('md_news')}.*");

        return $this->db->get_where('md_news', ['status' => 1])->result();
    }

    public function getJobs()
    {
        $career = $this->db->select("{$this->db->dbprefix('md_careers')}.*")
        ->from('md_careers')
        ->order_by('id','desc')
        ->get()
        ->result();
    ...
    public function getLastCareers($limit)
    {
        $career = $this->db->select("{$this->db->dbprefix('md_careers')}.*")
        ->from('md_careers')
        ->order_by('id','desc')
    ...
    {
        $query = $this->db->get_where('md_careers',['id' => $id]);
        if($query->num_rows() > 0){
            return $query->row();
        }
        return false;
    ...
    public function getPartnersForHeader()
    {
